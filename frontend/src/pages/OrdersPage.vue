...
const visibleOrders = computed(() => orders.orders.slice(0, page.value * pageSize));
const hasMoreOrders = computed(() => visibleOrders.value.length < orders.orders.length);
        ...
        <article class="rounded-[1.35rem] border border-ink bg-[#fffaf0] p-5">
          ...
          <div class="mt-4 divide-y divide-line rounded-2xl border border-line bg-paper">
            ...
            <div class="flex items-center justify-between gap-4 p-4">
              ...
              <p class="text-2xl font-black">{{ orders.orders.length }}</p>
            </div>
          </div>
